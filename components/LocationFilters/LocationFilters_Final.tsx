...
import { LocationOriginResolver } from '../../ApiSupabase/LocationOriginResolver';
...
export default function LocationFilters({
  ...
  useEffect(() => {
    if (resetComponent !== undefined) {
      ...
      const clearedFilters: ITournamentFilters = {
        ...
        lat: undefined, // Clear coordinates
        lng: undefined, // Clear coordinates
      };
  ...
  const handleZipCodeChange = (value: string) => {
    ...
    // Don't call updateFilters here - let the coordinate resolution effect handle it
  };
  ...
  // Coordinate resolution effect - resolves lat/lng from zip code and updates filters
  useEffect(() => {
    let isMounted = true; // Prevent state updates if component unmounts

    async function resolveCoordinatesAndUpdateFilters() {
      if (zipCode.trim() === '') {
        // Clear lat/lng if no zip code
        if (isMounted) {
          updateFilters(state, city, '', radius, undefined, undefined);
        }
        return;
      }

      try {
        const origin = await LocationOriginResolver.resolveOrigin({
          zip_code: zipCode,
        });

        if (isMounted) {
          if (origin) {
            // Update filters with resolved lat/lng coordinates
            updateFilters(
              state,
              city,
              zipCode,
            ...
            console.log(
              `[LocationFilters] Resolved ZIP ${zipCode} to coordinates: ${origin.latitude}, ${origin.longitude}`,
            );
          } else {
            // If cannot resolve coordinates, still update with zip_code and radius
            updateFilters(state, city, zipCode, radius, undefined, undefined);
            console.warn(
              `[LocationFilters] Could not resolve coordinates for ZIP ${zipCode}`,
            );
          }
        }
      } catch (error) {
        console.error(
          `[LocationFilters] Error resolving coordinates for ZIP ${zipCode}:`,
          error,
        );
        if (isMounted) {
          // On error, still update with zip_code and radius
          updateFilters(state, city, zipCode, radius, undefined, undefined);
        }
      }
    }

    resolveCoordinatesAndUpdateFilters();

    // Cleanup function to prevent state updates after unmount
    return () => {
      isMounted = false;
    };
  }, [zipCode, radius, state, city]); // Include state and city to ensure proper updates
