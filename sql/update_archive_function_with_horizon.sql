-- Update Archive Function to Use Horizon-Based Generation
-- Replaces the old generate_recurring_tournaments() call with generate_recurring_tournaments_horizon()
...
BEGIN
    ...
    -- Generate new recurring tournaments using horizon-based approach
    -- Sum up all tournaments created across all series
    FOR generation_result IN 
        SELECT * FROM generate_recurring_tournaments_horizon()
    LOOP
        generated_tournaments_count := generated_tournaments_count + generation_result.tournaments_created;
    END LOOP;
...
'Archives tournaments on their tournament date (same day archival) and generates new recurring tournament instances using horizon-based approach (60 days ahead). Prevents duplicates and past tournaments.';
...
FROM pg_proc
WHERE proname = 'archive_expired_tournaments'
LIMIT 1;
