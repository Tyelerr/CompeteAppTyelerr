...
-- The archive job can set status to 'archived' but must NEVER change recurring_template_status
...
-- Alternative: Simple UPDATE approach (if not using archive table)
-- This can be used instead of the function above if you prefer to just update status
...
WHERE start_date < CURRENT_DATE 
AND status = 'active';
-- Note: This does NOT touch recurring_template_status at all
*/
