-- FINAL FIX: Generator Should UPSERT (Reactivate Archived Tournaments)
-- Problem: ON CONFLICT DO NOTHING skips archived future tournaments
-- Solution: Use ON CONFLICT DO UPDATE to reactivate them
...
BEGIN
    ...
    LOOP
        ...
        -- Find latest ACTIVE child (ignore archived ones)
        ...
        WHILE cursor_date < target_date LOOP
            ...
            IF cursor_date >= CURRENT_DATE THEN
                ...
                -- CRITICAL FIX: Use UPSERT to reactivate archived tournaments
                BEGIN
                    ...
                    DO UPDATE SET
                        ...
                        recurring_template_status = EXCLUDED.recurring_template_status;  -- Update template status too

                    -- Count as created/reactivated
                    new_count := new_count + 1;
                    total_created := total_created + 1;

                EXCEPTION WHEN OTHERS THEN
                    RAISE WARNING 'Error creating/reactivating tournament for series % on date %: %', 
        ...
        IF new_count > 0 THEN
            RETURN QUERY SELECT
                ...
                new_count,
                format('Created/reactivated %s tournaments for series %s', 
                    new_count, 
                    master_record.tournament_name)::TEXT;
        END IF;
    END LOOP;
...
Only looks at ACTIVE children when finding latest.
CRITICAL: Uses UPSERT to reactivate archived future tournaments instead of skipping them.
This ensures the 60-day horizon is always maintained.';
