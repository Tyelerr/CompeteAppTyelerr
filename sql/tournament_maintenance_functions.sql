...
COMMENT ON FUNCTION archive_expired_tournaments() IS 'Archives tournaments on their tournament date (same day archival) and generates new recurring tournament instances';
COMMENT ON FUNCTION generate_recurring_tournaments() IS 'Generates new recurring tournament instances to maintain at least 4 future tournaments per series';
COMMENT ON FUNCTION archive_tournament_manual(UUID, TEXT, TEXT) IS 'Manually archives a tournament (for deletions) with optional user tracking';
COMMENT ON FUNCTION get_tournament_archival_stats() IS 'Returns statistics about tournament archival and current active tournaments';
