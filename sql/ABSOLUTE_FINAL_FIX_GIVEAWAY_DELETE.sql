-- ============================================================================
-- ABSOLUTE FINAL FIX FOR GIVEAWAY DELETE/ARCHIVE ERROR
-- ============================================================================
-- This adds ALL missing columns to BOTH archive tables
-- Run this ENTIRE file in Supabase SQL Editor
-- ============================================================================

-- STEP 1: Add ALL v1 columns to giveaways_archive table
-- ============================================================================
...
-- STEP 2: Add ALL missing columns to giveaway_entries_archive table
-- ============================================================================
-- This includes BOTH the old fields (name, email, phone, birthday) 
-- AND the new v1 fields (status, disqualified_*)
...
ADD COLUMN IF NOT EXISTS agreed_to_terms BOOLEAN,
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM giveaways WHERE id = v_giveaway_id) THEN
        ...
        -- Step 1: Archive all entries with ALL columns
        INSERT INTO giveaway_entries_archive (
            id, giveaway_id, user_id, 
            name, email, phone, birthday, agreed_to_terms, entry_number,
            status, disqualified_reason, disqualified_at, disqualified_by,
            created_at, removal_date, removal_reason, archived_giveaway_id
        )
        SELECT 
            ge.id, ge.giveaway_id, ge.user_id,
            ge.name, ge.email, ge.phone, ge.birthday, ge.agreed_to_terms, ge.entry_number,
            ge.status, ge.disqualified_reason, ge.disqualified_at, ge.disqualified_by,
            ge.created_at, NOW(), reason, v_giveaway_id
...
BEGIN
    ...
    RAISE NOTICE '  - giveaway_entries_archive (added 10 columns total)';
    RAISE NOTICE '    * name, email, phone, birthday, agreed_to_terms, entry_number';
    RAISE NOTICE '    * status, disqualified_reason, disqualified_at, disqualified_by';
