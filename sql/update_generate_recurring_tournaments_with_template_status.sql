-- Step 2: Update Generator Function to Use recurring_template_status
-- This allows the generator to work even when master tournaments are archived
...
BEGIN
    ...
    -- CRITICAL CHANGE: Use recurring_template_status instead of status
    FOR master_record IN
        SELECT * FROM tournaments
        WHERE is_recurring_master = true
        AND is_recurring = true
        AND recurring_template_status = 'active'  -- NEW: Use template status, not instance status
    LOOP
        new_count := 0;
            ...
            IF cursor_date >= CURRENT_DATE THEN
                ...
                BEGIN
                    INSERT INTO tournaments (
                        ...
                        recurring_template_status,  -- NEW: Copy template status to children
                        reports_to_fargo,
                        is_open_tournament,
                        race_details,
                        number_of_tables,
                        ...
                        master_record.recurring_template_status, -- NEW: Inherit template status
                        ...
                        'active', -- status (instance status, always active for future tournaments)
    ...
    IF total_created = 0 THEN
        RETURN QUERY SELECT
            NULL::TEXT,
            0,
            'No new tournaments needed - all active series are up to date'::TEXT;
    END IF;
...
Uses recurring_template_status to determine which series should generate (not instance status).
This allows generation to continue even when master tournaments are archived.
