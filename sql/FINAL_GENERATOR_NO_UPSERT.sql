-- FINAL GENERATOR - No UPSERT, just skip duplicates
    ...
    LOOP
            ...
            IF next_date >= CURRENT_DATE THEN
                ...
                IF NOT duplicate_exists THEN
                    ...
                    SELECT 
                        ...
                        uuid,  -- KEEP MASTER'S UUID (creator profile)
                        side_pots,
                        thumbnail_type,
                        'active',  -- status
                        NULL,  -- deleted_at
                        new_id_num,  -- new id_unique_number
                        venue_lat,
                        venue_lng,
                        custom_equipment,
                        point_location,
        ...
        IF created_count > 0 THEN
            RETURN QUERY SELECT
                ...
                created_count,
                format('%s tournaments created for %s', created_count, master_rec.tournament_name)::TEXT;
        END IF;
    END LOOP;
...
Uses INSERT...SELECT to copy all columns from master.
Checks for duplicates before inserting, reactivates archived tournaments.
