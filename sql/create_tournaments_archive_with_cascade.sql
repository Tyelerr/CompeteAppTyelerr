-- Create a simple tournaments archive table with proper foreign key handling
...
-- This version handles likes deletion properly
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM tournaments WHERE id = tournament_id) THEN
        ...
        DELETE FROM likes WHERE tournament_id_fkey = tournament_id;
...
BEGIN
    ...
    LOOP
        -- First, delete all likes associated with this tournament
        DELETE FROM likes WHERE tournament_id_fkey = tournament_record.id;
