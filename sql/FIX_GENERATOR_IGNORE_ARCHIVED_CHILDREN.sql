-- Fix Generator to Ignore Archived Children When Finding Latest
-- The generator should only look at ACTIVE children to determine what to generate next
-- This ensures it keeps generating to maintain the 60-day horizon even when old tournaments are archived
...
BEGIN
    ...
    -- Use recurring_template_status instead of status
    ...
    LOOP
        ...
        -- CRITICAL FIX: Find latest ACTIVE child (ignore archived ones)
        -- This ensures we keep generating to maintain the horizon
        ...
        AND status = 'active';  -- NEW: Only look at active children

        -- Start from latest active child date or master date, whichever is later
            ...
            IF cursor_date >= CURRENT_DATE THEN
                ...
                BEGIN
                    INSERT INTO tournaments (
                        ...
                        master_record.recurring_template_status,
                        ...
                        'active', -- status (always active for future tournaments)
        ...
        IF new_count > 0 THEN
            RETURN QUERY SELECT
                ...
                new_count,
                format('Created %s tournaments for series %s (master status: %s, template status: %s)', 
                    new_count, 
                    master_record.tournament_name,
                    master_record.status,
                    master_record.recurring_template_status)::TEXT;
        END IF;
    END LOOP;
    ...
    IF total_created = 0 THEN
        RETURN QUERY SELECT
            NULL::TEXT,
            0,
            'No new tournaments needed - all active series are up to date with 60-day horizon'::TEXT;
    END IF;
...
Uses recurring_template_status to determine which series should generate.
CRITICAL: Only looks at ACTIVE children when finding latest, so archived tournaments dont block generation.
This ensures the 60-day horizon is maintained even as old tournaments are archived.';
