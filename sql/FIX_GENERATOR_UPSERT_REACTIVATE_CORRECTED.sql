    ...
    LOOP
        ...
        WHILE cursor_date < target_date LOOP
            ...
            IF cursor_date >= CURRENT_DATE THEN
                ...
                EXCEPTION WHEN OTHERS THEN
                    RAISE WARNING 'Error for series % on date %: %', 
        ...
        IF new_count > 0 THEN
            RETURN QUERY SELECT
                ...
                new_count,
                format('Created/reactivated %s tournaments for %s', new_count, master_record.tournament_name)::TEXT;
        END IF;
    END LOOP;
...
'Generates recurring tournaments with 60-day horizon. Uses UPSERT to reactivate archived future tournaments.';
